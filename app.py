...
def core_sign(pdf_file):
    ...
    # 3. Insertion dans le PDF (QR + Métadonnées)
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    # On cache la preuve dans les métadonnées du document
...
def verify_pdf_web():
    ...
    try:
        qr = json.loads(qr_data)
        # On vérifie si la signature RSA du QR est valide avec notre clé publique
        public_key.verify(
        ...
        return "<h2>✅ Authentification réussie via QR</h2><a href='/'>Retour</a>"
    except:
        return "<h2>❌ Signature ou Document invalide</h2><a href='/'>Retour</a>"
...
def verify_qr_web():
    ...
    try:
        ...
        return "<h2>✅ QR Code Certifié par le serveur</h2>"
    except:
        return "<h2>❌ QR Code non reconnu</h2>"
